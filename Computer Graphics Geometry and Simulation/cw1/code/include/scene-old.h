#ifndef SCENEOLD_HEADER_FILE
#define SCENEOLD_HEADER_FILE
...
public:
  ...
  void add_mesh(const MatrixXd& V, const MatrixXi& F, const MatrixXi& T, const double density, const bool isFixed, const RowVector3d& COM, const RowVector4d& orientation){ 
    ...
    // TODO: Update cellSize when loading
    // // get colume of mesh 
    // double volume = 
    // // Set cell size to the smallest object
    // Grid::cellSize = min(Grid::cellSize, currV.get_diag().minCoeff());
  }
  ...
  void update_scene(double timeStep, double CRCoeff, int maxIterations, double tolerance){
    ...
    double depth;
    RowVector3d penPosition, contactNormal;
    for (int i=0;i<meshes.size();i++)
    ...
    // TODO: More efficienct contraint resolution
    int currIteration=0;
